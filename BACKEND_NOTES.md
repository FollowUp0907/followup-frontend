...
# 회원가입 이메일 인증 요청 (2026-09-21)

회원가입할 때 **입력한 이메일로 6자리 인증번호를 보내고, 그 번호를 맞혀야 가입되도록**
바꾸려고 합니다. 프론트는 아래 규격에 맞춰 **이미 다 만들어 뒀습니다.**
백엔드에서 API 두 개만 열어 주시면 됩니다.

메일 속 링크를 누르는 방식이 아니라 **번호 입력 방식**으로 정했습니다. 이유는 두 가지입니다.

- 백엔드가 프론트 주소를 몰라도 됩니다. Vercel 프리뷰 배포는 주소가 매번 바뀌어서,
  링크 방식이면 메일에 넣을 주소를 그때그때 맞춰야 합니다.
- 메일을 휴대폰으로 열어도 PC 에서 가입을 그대로 이어갈 수 있습니다.
...
```json
{ "expiresIn": 300, "resendAfter": 60 }
```

| 필드 | 뜻 | 없으면 프론트 기본값 |
| --- | --- | --- |
| `expiresIn` | 인증번호 유효시간(초) | 300 (5분) |
| `resendAfter` | 재발송까지 기다릴 시간(초) | 60 |

두 필드 다 **선택**입니다. 응답 본문이 비어 있어도(204) 프론트는 기본값으로 동작합니다.
다만 화면에 남은 시간을 그대로 보여주고 있어서, 실제 값과 맞춰 주시면 좋습니다.

**에러**

| 상태 | 언제 | 화면에 뜨는 것 |
| --- | --- | --- |
| 409 | 이미 가입된 이메일 | `message` 를 그대로 보여줍니다 |
| 429 | 재발송 간격 안에 또 요청 | `message` 를 그대로 보여줍니다 |
| 400 | 이메일 형식이 아님 | 〃 |

에러 본문은 기존 `ErrorResponse`(`{ code, message }`) 그대로 쓰시면 됩니다.
프론트는 `message` 를 그대로 띄웁니다.

## 2. 인증번호 확인

```
POST /api/auth/email/verification-codes/verify
Content-Type: application/json
...
```json
{ "verificationToken": "…", "expiresIn": 1800 }
```

`verificationToken` 은 **"이 주소는 인증됐다"는 증표**입니다. 형식은 자유입니다
(랜덤 문자열 + Redis/DB, 또는 서명된 짧은 JWT). 프론트는 내용을 열어보지 않고
회원가입 요청에 그대로 실어 보냅니다.
...

{
  ...
  "email": "you@example.com",
  "password": "…",
  "verificationToken": "…"   ← 추가
}
```

- `verificationToken` 이 없거나, 만료됐거나, **토큰에 적힌 이메일과 `email` 이 다르면 400**
  으로 막아 주세요. 마지막 방어선은 백엔드입니다. 프론트에서 버튼을 잠가 두긴 했지만
  요청은 얼마든지 직접 만들 수 있습니다.
- 토큰은 **한 번 쓰면 폐기**해 주세요.

## 부탁드리는 안전장치

1. **인증번호는 해시로 저장** — 평문으로 두면 DB 를 보는 사람이 남의 가입을 가로챌 수 있습니다.
2. **시도 횟수 제한** — 같은 번호에 5회 틀리면 무효로 하고 재발송을 받게 해 주세요.
   6자리는 100만분의 1이지만, 제한이 없으면 자동으로 다 넣어볼 수 있습니다.
3. **발송 제한** — 같은 이메일·같은 IP 기준으로 시간당 횟수를 막아 주세요.
   메일 서버가 스팸으로 분류되는 걸 막는 목적도 있습니다.
4. **이미 가입된 이메일이면 메일을 보내지 말고 409** — 괜한 메일이 갑니다.
5. `verificationToken` 유효시간은 **30분 정도**면 충분합니다. 번호를 맞힌 뒤
   비밀번호를 정하는 데 걸리는 시간만 있으면 됩니다.

## 메일 내용 (제안)

```
제목: [FollowUp] 이메일 인증번호 123456

FollowUp 회원가입 인증번호입니다.
...
프론트는 `POST /api/auth/email/verification-codes` 가 **404·405·501** 로 오면
"백엔드에 아직 인증이 없다"고 보고 **인증 단계를 통째로 숨기고 예전처럼 바로 가입**시킵니다.
그래서 지금 프론트를 먼저 배포해도 가입이 막히지 않습니다.
백엔드가 올라가는 순간부터 인증이 자동으로 켜집니다.

반대로 백엔드가 먼저 올라가도, 옛 프론트는 `verificationToken` 을 안 보냅니다.
**두 쪽이 다 올라간 뒤에** `verificationToken` 없는 가입을 400 으로 막아 주세요.
그 전까지는 토큰이 없어도 통과시켜 주셔야 합니다.

## 프론트 동작 (참고)

1. 이메일을 형식에 맞게 입력하면 **[인증 메일 보내기]** 버튼이 열립니다.
2. 보내고 나면 인증번호 칸이 나오고, 남은 시간(5:00)과 재발송 대기(1:00)를 셉니다.
3. 번호를 맞히면 이메일 칸 아래 **✓ 이메일 인증 완료** 가 뜨고 [회원가입] 이 열립니다.
4. 이메일을 한 글자라도 고치면 인증은 무효가 되고 처음부터 다시 합니다.
   (인증한 주소와 가입하는 주소가 달라지면 안 되니까요)
5. 구글 로그인은 그대로입니다 — 구글이 이미 `email_verified` 를 보장합니다.
