...
## 확인 완료 (2026-09-18 백엔드 회신)

**1. `assigneeUserIds: []` 는 "담당자 전원 해제" 로 처리됩니다.**
`ActionItemUpdateReqDto` 에 이렇게 문서화되어 있다고 합니다 —
*"null 이면 그대로 두고, 값이 있으면(빈 리스트 포함) 담당자 전체를 이 목록으로 교체한다"*

| 보낸 것 | 결과 |
| --- | --- |
| 필드 자체를 안 보냄 (null) | **기존 담당자 그대로** |
| `[]` | **담당자 전원 해제** |
| `[7, 8]` | 그 둘로 교체 |

> ⚠️ **프론트에서 주의할 점** — 부분 수정(PATCH)에 `assigneeUserIds` 를 습관적으로
> 넣으면 안 됩니다. 담당자를 안 건드릴 때는 **필드를 빼야** 합니다.
> 지금 코드는 맞게 되어 있습니다: 수정 폼은 전체 목록을 알고 있으니 항상 보내고,
> 보드에서 카드를 끌어 상태만 바꿀 때는 `{ status }` 만 보냅니다.

예전에 `assigneeUserId: null` 을 무시하고 값을 안 지우던 문제와는 다른 경우입니다.
null 과 `[]` 를 명확히 구분해서 처리한다고 하니 안전합니다.

**2. `DraftActionItem.assigneeName` 은 그대로 단일 문자열입니다.**
AI 가 회의록에서 뽑아낸 자유 텍스트라 담당자 지정과 성격이 다릅니다. 확정할 때
실제로 몇 명을 넣을지는 `assigneeUserIds` 가 정하고, `assigneeName` 은
"AI 가 이렇게 읽었다" 는 참고용입니다. 프론트도 그대로 참고용으로만 씁니다.
...
# ~~회원가입 이메일 인증 요청 (2026-09-21)~~ → 취소되었습니다

회원가입에 이메일 인증을 붙이려던 요청은 **없던 일로 해 주세요.** 프론트에서도 되돌렸습니다.
인증이 필요한 곳은 회원가입이 아니라 **구성원 초대** 였습니다. 아래 요청으로 대신합니다.
(혹시 이미 만드셨다면 알려 주세요 — 인증번호 발송·검증 부분은 초대 메일에 그대로 쓸 수 있습니다.)

---

# 구성원 초대 요청 (2026-09-21)

지금은 **이미 FollowUp 에 가입한 사람만** 구성원으로 넣을 수 있습니다.
이걸 **이메일로 초대**하는 방식으로 바꾸려고 합니다.

- 이메일을 넣으면 그 주소로 **초대 메일**이 갑니다.
- 받은 사람이 메일의 링크로 **수락해야** 구성원이 됩니다.
- **FollowUp 계정이 없어도** 초대할 수 있습니다. 메일에서 가입을 마치면 그 프로젝트로 자동 합류합니다.

프론트는 아래 규격대로 **이미 다 만들어 뒀습니다.** 화면 두 곳(설정 > 구성원, 새 회의 > 구성원 추가)이
같은 창을 씁니다.

## 1. 초대 보내기

```
POST /api/project/{projectId}/invitations
{ "email": "newbie@example.com" }
```

**201** — 만든 초대를 그대로 돌려주세요.
...
```
GET /api/project/{projectId}/invitations  →  InvitationResDto[]
```

`PENDING` 만 주셔도 되고 전부 주셔도 됩니다. 프론트가 `status === 'PENDING'` 만 골라 씁니다.
설정 > 구성원 목록에 **"수락 대기 중"** 줄로 함께 보여줍니다.

## 3. 재발송 / 취소

```
POST   /api/project/{projectId}/invitations/{invitationId}/resend   → 204
DELETE /api/project/{projectId}/invitations/{invitationId}          → 204
```

취소하면 그 링크는 더 이상 동작하지 않아야 합니다.
...
메일 링크로 들어온 사람에게 **"어느 프로젝트에 초대받았는지"** 를 로그인 전에 보여주기 위한 것입니다.
비밀번호나 다른 구성원 정보는 넣지 말아 주세요. 토큰만 알면 누구나 볼 수 있는 응답입니다.
...
- 로그인한 계정의 이메일이 **초대받은 주소와 같아야** 합니다. 다르면 **403**.
- 만료·취소된 초대는 **410** 또는 **400**.
- 성공하면 4번과 같은 모양(`status: "ACCEPTED"`)을 주시면, 프론트가 `projectId` 로 바로 이동시킵니다.

## 6. 가입할 때 자동 합류

가입 안 한 사람이 초대를 받은 경우, 프론트는 이렇게 움직입니다.

```
초대 메일 링크 → /invite/{token} → [회원가입] → 가입 완료 → 다시 /invite/{token} → 수락
```

그래서 **가입 API 는 그대로 두셔도 됩니다.** 다만 가입에 쓴 이메일이 초대받은 주소와 같으면
수락이 통과해야 합니다 (5번 규칙 그대로).

## 메일에 넣을 링크

```
{APP_ORIGIN}/invite/{token}
```
...
1. **토큰은 추측할 수 없는 난수**로, DB 에는 해시로 저장해 주세요.
2. **한 번 수락하면 폐기** — 같은 링크로 두 번 들어올 수 없게.
3. **만료 7일** 정도면 충분합니다.
4. **정원(10명) 검사에 대기 중인 초대도 포함**해 주세요. 프론트도 그렇게 세고 있습니다
   (구성원 수 + PENDING 초대 수).
5. **발송 제한** — 같은 프로젝트에서 같은 주소로 반복 발송되지 않게.

## 중요 — 401 대신 404 를 주셨으면 합니다

우리 백엔드는 지금 **매핑되지 않은 경로에도 401** 을 줍니다. 프론트는 그걸 "세션 만료" 로 보고
로그아웃시키게 되어 있어서, 새 기능을 붙일 때마다 멀쩡한 사용자가 튕길 위험이 있습니다.

프론트에서는 초대 관련 호출에 한해 "401 이 와도 로그아웃하지 않는다" 는 표시를 달아 막아 뒀지만,
**없는 경로는 404** 로 주시는 편이 서로 안전합니다. 가능하면 확인 부탁드립니다.

## 배포 순서는 맞추지 않아도 됩니다

초대 목록 조회가 **404·405·501·(코드 없는)401·403** 으로 오면, 프론트는 "아직 초대가 없는 백엔드"로 보고
**예전 방식(가입한 계정을 바로 추가)** 그대로 동작합니다. 버튼 이름도 "구성원 추가" 로 돌아갑니다.
그래서 지금 프론트를 먼저 배포해도 구성원 추가가 막히지 않고, 백엔드가 올라가는 순간 초대로 바뀝니다.

## 프론트 동작 (참고)

1. 설정 > 구성원, 새 회의 > 구성원 추가 — 두 곳 모두 같은 초대 창을 씁니다.
2. 보낸 초대는 구성원 목록에 **"수락 대기 중"** 줄로 보이고, **재발송 / 취소** 버튼이 붙습니다.
3. 정원은 구성원 + 대기 중인 초대를 합쳐 셉니다. (예: 2명 + 대기 2건 = 4/10명)
4. 초대 링크로 들어오면 `/invite/{token}` 화면이 뜹니다 — 로그인 전에는 프로젝트 이름과
   "이 주소로 로그인해 주세요" 안내, 로그인 후에는 [초대 수락] 버튼.
5. 다른 계정으로 로그인돼 있으면 수락 버튼을 잠그고 어느 계정으로 들어와야 하는지 알려 줍니다.

---

# 추가 요청 3건 (2026-09-21 오후)

초대 API 잘 받았습니다. 프론트에 붙여서 확인했고, 규격대로 정확히 나왔습니다.
(`InvitationResDto.status` 가 `PENDING / ACCEPTED / CANCELLED` 인 것도 프론트에 맞춰 뒀습니다.)

아래 세 가지만 더 부탁드립니다.

## 1. 초대를 수락하면 그 프로젝트 구성원들에게 알림

새 사람이 합류한 걸 나머지 구성원이 알 수 있게, `POST /api/invitations/{token}/accept` 가
성공할 때 **그 프로젝트의 기존 구성원 전원에게** 알림을 하나씩 만들어 주세요.
(합류한 본인은 제외)

기존 `NotificationResDto` 를 그대로 쓰되, `type` 에 값을 하나 추가하면 됩니다.
...
```json
{
  ...
  "taskTitle": "반서현",
  "type": "MEMBER_JOINED",
  "remindAt": "2026-09-21T14:03:00",
  "createdAt": "2026-09-21T14:03:00"
}
```

| 필드 | 넣을 것 |
| --- | --- |
| `type` | `MEMBER_JOINED` (enum 에 추가) |
| `taskTitle` | **합류한 사람 이름** — 알림 목록의 제목 줄에 그대로 보입니다 |
| `projectId` | 합류한 프로젝트 |
| `actionItemId` | `null` (업무와 무관한 알림) |

프론트는 이미 받을 준비가 되어 있습니다. 목록에 사람 아이콘과 함께
**"반서현 / 초대를 수락해 합류했습니다"** 로 보이고, 눌러도 업무 화면이 아니라
구성원 목록(설정)으로 갑니다. SSE 로도 그대로 흘려 주시면 즉시 뜹니다.

`actionItemId` 가 필수라 `null` 이 어려우시면 알려 주세요. 프론트에서 0 도 받게 하겠습니다.

## 2. 내 계정 — 이름 변경 / 회원 탈퇴

우측 상단 프로필에 톱니를 넣어 **이름 수정**과 **회원 탈퇴**를 그 자리에서 하게 만들었습니다.
API 가 없어서 지금은 "이 기능은 서버에 아직 준비되지 않았습니다" 라고 뜹니다.

```
PATCH /api/users/me
{ "name": "장은호" }
→ 200 { "userId": 1, "name": "장은호", "email": "..." }
```
...
- 되돌릴 수 없는 삭제입니다. 화면에서는 한 번 더 확인을 받고 있습니다.
- **정해 주셔야 할 것**: 탈퇴한 사람이 만든 프로젝트와, 맡고 있던 업무를 어떻게 할지.
  지금 화면에는 *"내가 만든 프로젝트와 회의록도 함께 사라집니다. 맡고 있던 업무는 담당자 없이 남습니다"*
  라고 적어 뒀습니다. 백엔드 처리가 다르면 알려 주세요. **문구를 그쪽에 맞추겠습니다.**
  (사실과 다른 안내가 제일 나쁩니다)

## 3. 프로젝트 나가기 — 본인 스스로 빠질 수 있게

소유자가 아닌 구성원이 프로젝트에서 나갈 수 있도록 설정 화면에 버튼을 넣었습니다.
호출은 **기존 API 를 그대로** 씁니다.

```
DELETE /api/project/{projectId}/member/{myUserId}
```

- 지금 이 API 가 **OWNER 만** 호출할 수 있게 되어 있다면, **본인 자신을 지우는 경우는 허용**해 주세요.
- 소유자는 나갈 수 없어야 합니다(프로젝트에 주인이 없어지므로). 400/403 으로 막아 주시면
  프론트에서 메시지를 그대로 보여줍니다. 화면에서도 소유자에게는 버튼을 띄우지 않습니다.
- 나가도 **그 사람이 맡던 업무는 그대로 남아야** 합니다. 화면에 그렇게 안내하고 있습니다.

## 그리고 — 없는 경로에는 401 말고 404 를

지난번에도 적었는데 이번에 또 걸렸습니다. 매핑되지 않은 경로가
`401 {"code":"UNAUTHORIZED"}` 로 와서, 프론트는 이걸 "세션 만료" 와 구분할 수 없습니다.

지금은 프론트에서 **"401 인데 코드가 일반적인 UNAUTHORIZED/FORBIDDEN 이면 없는 API 로 본다"** 는
규칙으로 우회하고 있습니다. 동작은 하지만, 진짜로 세션이 끊긴 경우와 구분이 흐려집니다.
`SecurityConfig` 에서 인증 실패와 "핸들러 없음(NoHandlerFoundException)" 을 나눠 주시면
서로 깔끔해집니다. 여유 되실 때 부탁드립니다.

---

# 웹 푸시 (2026-09-22)

푸시 API 잘 받았습니다. 프론트는 아래대로 **붙여 뒀습니다.** 값 세 개만 더 주시면 켜집니다.

## 아직 못 받은 값

| 값 | 어디서 | 비고 |
| --- | --- | --- |
| `apiKey` | 콘솔 > 프로젝트 설정 > 일반 > 내 앱(웹) | 공개값 |
| `messagingSenderId` | 〃 | 공개값 |
| `appId` | 〃 | 공개값 |

받은 것: `projectId` = `followup-f4335`, VAPID 공개키. (authDomain 은 관례대로
`followup-f4335.firebaseapp.com` 으로 넣어 뒀습니다 — 다르면 알려 주세요)

## ⚠️ 서비스 계정 JSON 은 서버 전용입니다

`followup-f4335-firebase-adminsdk-*.json` 을 받았는데, 이건 프론트에서 쓰는 값이 아니라
**Firebase 프로젝트를 관리자 권한으로 조작할 수 있는 비밀키**입니다. 브라우저 번들에 들어가면
누구나 읽을 수 있습니다. 프론트는 이 파일을 쓰지 않았고, 저장소에도 넣지 않았습니다.
...
1. 알림 종을 열면 맨 위에 **"브라우저 알림 켜기"** 가 나옵니다. **화면에 들어오자마자 권한을
   묻지 않습니다** — 묻자마자 거부당하면 다시 물을 수 없고, 브라우저도 그런 사이트에 불이익을 줍니다.
2. 허용하면 FCM 토큰을 받아 `POST /api/push/subscriptions` 로 보냅니다.
3. **앱을 열 때마다 토큰을 다시 등록합니다.** 토큰은 브라우저가 조용히 갱신하기 때문에
   한 번 보내고 마는 것으로는 부족합니다. 같은 토큰이 다시 와도 괜찮게 처리해 주세요.
4. 로그아웃하면 `DELETE` 로 구독을 지우고 로컬 토큰도 폐기합니다.
5. 앱이 켜져 있을 때 온 푸시는 배너를 띄우지 않고 **알림 목록만 새로 고칩니다.**
   말씀하신 대로 SSE 가 더 빠르고, 배너가 두 번 뜨지 않습니다.
6. 알림을 누르면 `data.projectId` / `data.actionItemId` 로 보냅니다. `actionItemId` 가 빈
   문자열이면(합류 알림 등) 프로젝트 화면으로 갑니다. 이미 열린 탭이 있으면 그 탭을 씁니다.
7. 설정값이 비어 있으면 **"브라우저 알림 켜기" 자체가 나타나지 않습니다.** Firebase SDK 도
   내려받지 않습니다(실제로 켤 때만 불러옵니다). 지금 배포해도 아무 영향이 없습니다.

---

# 500 오류 확인 요청 — `GET /api/project/{id}/meetings` (2026.09.22)

프로젝트 **13번**의 회의 목록에서 **500** 이 계속 납니다.

```
GET /api/project/13/meetings  →  500
```

다른 프로젝트에서는 나지 않고 있어서, **그 프로젝트의 특정 회의 행 하나가 원인**일 가능성이 큽니다.
...
이 오류를 보다가 **화면이 거짓말하는 문제**를 하나 찾아서 같이 고쳤습니다.

- 예전: 목록을 못 불러오는 동안 **"아직 기록된 회의가 없어요"** 라는 빈 상태가 보였습니다.
  회의가 있는 프로젝트인데도 없다고 표시돼서, 데이터가 사라진 것처럼 보였습니다.
- 지금: 한 번도 성공하지 못한 동안에는 기다리는 화면을, 실패가 확정되면
  **"회의 목록을 불러오지 못했습니다 + 다시 시도"** 를 보여줍니다.
- 회의 목록, 프로젝트 목록, 후속 업무 목록 세 곳 모두 같게 맞췄습니다.

재시도는 5xx 에 대해 **2회까지만** 하고 멈춥니다. 무한히 두드리지 않습니다.
